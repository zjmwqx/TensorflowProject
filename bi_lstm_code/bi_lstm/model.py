...
class BiLstmModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        if (config.bi_direction):  # BiLSTM
            self._cost, self._logits, self._accuracy = self._bilstm_model(self.is_training, inputs, self._targets,
                                                                          config)
        else:  # LSTM
            self._cost, self._logits, self._final_state, self._initial_state, self._accuracy = \
                self._lstm_model(self.is_training, inputs, self._targets, config)
    ...
    def _lstm_model(self, is_training, inputs, targets, config):
        '''
        ...
        if is_training and config.keep_prob < 1:
            # 如果是训练状态且keep_prob小于1，则进行dropout防止过拟合
        ...
        with tf.variable_scope("pos_lstm"):
            for time_step in range(num_steps):
                ...
                (cell_output, state) = cell(inputs[:, time_step, :],
                                            state)  # inputs[batch_size, time_step, hidden_size]
        ...
        return cost, logits, state, initial_state, accuracy

    def _bilstm_model(self, is_training, inputs, targets, config):
        '''
        ...
        lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True)  # 正向
        if is_training and config.keep_prob < 1:
            # 如果是训练状态且keep_prob小于1，则进行dropout防止过拟合
            lstm_fw_cell = tf.contrib.rnn.DropoutWrapper(
                    lstm_fw_cell, output_keep_prob=config.keep_prob)

        lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True)  # 反向
        if is_training and config.keep_prob < 1:
            # 如果是训练状态且keep_prob小于1，则进行dropout防止过拟合
            lstm_bw_cell = tf.contrib.rnn.DropoutWrapper(
                    lstm_bw_cell, output_keep_prob=config.keep_prob)

        cell_fw = tf.contrib.rnn.MultiRNNCell([lstm_fw_cell] * num_layers, state_is_tuple=True)  # 正向
        cell_bw = tf.contrib.rnn.MultiRNNCell([lstm_bw_cell] * num_layers, state_is_tuple=True)  # 反向
